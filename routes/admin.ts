...
router.patch("/nivel/:id/:nivel", verificaToken, async (req, res) => {
  const { id, nivel } = req.params;


  const nivelNum = Number(nivel);
  if (isNaN(nivelNum) || nivelNum < 1 || nivelNum > 5) {
    return res.status(400).json({ erro: "Nível deve ser entre 1 e 5" });
  }
  ...
  try {
    ...
    res.status(400).json({ erro: "Não foi possível alterar o nível do admin" });
  }
});
