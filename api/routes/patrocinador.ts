...
const patrocinadorSchema = z.object({
  nome: z.string().min(10, {
    message: "Nome do patrocinador deve possuir, no mínimo, 10 caracteres"
  }),
  email: z.string().email({message: "Informe um e-mail válido"}),
  senha: z.string(),
  cidade: z.string(),
  idade: z.number()
    .min(18, { message: "Idade, no mínimo, 18 anos" })
})

router.get("/", async (req, res) => {
  try {
    const patrocinador = await prisma.patrocinador.findMany()
...
router.post("/", async (req, res) => {
  ...
  const valida = patrocinadorSchema.safeParse(req.body)
  if (!valida.success) {
  ...
  const { nome, email, cidade, idade } = valida.data
 
  try {
    const patrocinador = await prisma.patrocinador.create({
      data: { nome, email, senha: hash, cidade, idade: Number(idade) }
    })
